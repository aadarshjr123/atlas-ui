...
describe("Message", () => {
  it("renders user message aligned to the right", () => {
    render(<Message author="user">Can we approve this?</Message>);
  ...
  it("renders assistant message with action buttons by default", () => {
    render(<Message author="assistant">Approved with review.</Message>);
  ...
  it("hides action buttons when actions is false", () => {
    render(<Message author="assistant" actions={false}>No actions</Message>);
    expect(screen.queryByLabelText("Copy response")).not.toBeInTheDocument();
  });

  it("does not show action buttons for user messages", () => {
    render(<Message author="user">Hello</Message>);
    expect(screen.queryByLabelText("Copy response")).not.toBeInTheDocument();
  });

  it("renders system messages", () => {
    render(<Message author="system">System message</Message>);
    expect(screen.getByText("System message")).toBeInTheDocument();
    expect(screen.getByRole("article")).toHaveAttribute("data-author", "system");
  });
});
